...
This is the deterministic M12 boundary harness. Both adapters invoke the existing `NativeSubagentRunner` with the same fixture session, request normalization, profile/resource validation, event mapping, result parsing, cancellation signal, and cleanup contract. The subprocess adapter only isolates that runner in a child process; it is not a second Ice loop. M12 and M13 are COMPLETE / VERIFIED / FROZEN from the canonical full-budget evidence chain below.
...
```bash
npm run ice:m12:preflight
npm run ice:m12:self-test
npm run ice:m12:bench -- --mode cold --repetitions 30
npm run ice:m12:bench -- --mode warm --warmups 5 --repetitions 100
npm run ice:m12:safety -- --repetitions 50
npm run ice:m12:compat -- --repetitions 3
npm run ice:m12:report -- --run-id 99f421f8-228b-4137-a44e-092895359713
npm run ice:m13:verify -- --run-ids 99f421f8-228b-4137-a44e-092895359713,d6222038-23e7-4ca9-a8ce-1770dc96db37,67457e73-cd91-452b-b0a0-154ebf94a369,430338fe-ddb8-45f6-a0ee-fdaaee129df9
```
...
- `cli-contract-matrix`: actual ICE/stock CLI help and invalid ICE backend-selection checks; stock `ice` remains free of ICE flags.
