...
export class TestSessionRuntime implements IceSessionRuntime {
	readonly disposed = new Deferred<void>();
	disposeCount = 0;
	readonly steers: PromptInput[] = [];
	private readonly stored: StoredSession;
	private readonly onDispose: () => void;
	private readonly listeners = new Set<(event: IceSessionRuntimeEvent) => void>();
	...
	async prompt(input: PromptInput): Promise<void> {
		if (this.getPhase() !== "idle") throw new IceServerError("busy", "A prompt is already running");
	...
	async steer(input: PromptInput): Promise<void> {
		if (this.getPhase() === "idle") throw new IceServerError("busy", "There is no active prompt to steer");
	...
	async abort(): Promise<void> {
		if (!this.pendingPrompt) throw new IceServerError("busy", "There is no active prompt to abort");
		this.pendingPrompt.done.resolve("aborted");
	}

	async setModel(model: ModelRef): Promise<void> {
		if (this.getPhase() !== "idle") throw new IceServerError("busy", "Session is busy");
		this.update({ model });
	}
	...
	emitError(error: IceServerError): void {
...
export class TestSessionBackend implements IceSessionBackend {
	...
	async createSession(options: CreateSessionOptions): Promise<IceSessionRuntime> {
		this.lastCreatedId = options.id;
		if (this.sessions.has(options.id)) throw new IceServerError("session_locked", "Session already exists");
		this.seed(options.id, options.name, options.cwd, options.model, options.thinkingLevel);
		return this.acquire(options.id);
	}

	async openSession(sessionId: string): Promise<IceSessionRuntime> {
		if (!this.sessions.has(sessionId)) throw new IceServerError("not_found", `Unknown session: ${sessionId}`);
		if (this.locked.has(sessionId)) throw new IceServerError("session_locked", `Session is locked: ${sessionId}`);
	...
	seed(
		id = "session-1",
		name = `Session ${id}`,
		cwd = "/tmp/ice-server-conformance",
