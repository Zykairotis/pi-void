...
Ice evals are behavioral, model-backed checks for Ice workflows. They adapt a real `AgentSession` to `vitest-evals`, run
it in isolated temporary project and agent directories, and attach native Ice session artifacts.
...
```bash
ICE_PROVIDER=openai ICE_MODEL=gpt-5.6-sol npm run eval
```
...
Authentication comes from Ice's normal `ModelRuntime`, including Ice subscription credentials and provider API-key
...
native Ice session JSONL attachments under `sessions/`. These files may contain prompts, responses, source code, and tool
output.

## Writing evals

Follow [`vitest-evals`](https://github.com/getsentry/vitest-evals) for general suite, judge, assertion, and normalized
trace guidance. Ice-specific evals use `createIceCodingAgentHarness(...)` from `src/ice-harness.ts`, with one harness bound
to each `describeEval(...)` suite:

```ts
import { expect } from "vitest";
import { describeEval } from "vitest-evals";
import { createIceCodingAgentHarness } from "./ice-harness.ts";

const harness = createIceCodingAgentHarness({ noTools: "all" });

describeEval("Ice smoke", { harness }, (it) => {
...
### Configuring the Ice harness

`createIceCodingAgentHarness(...)` accepts:
...
- `noTools`: Ice's tool-disable configuration.
...
```ts
const harness = createIceCodingAgentHarness({
...
prompt creates or changes Ice resources:

```ts
const result = await run([
	{ type: "prompt", content: "Create a Ice extension." },
	{ type: "reload" },
...
Use `output` to expose scenario-specific, JSON-safe behavior without adding that behavior to the generic Ice adapter:
...
Harnesses may differ by prompt, tools, skills, model, or any other Ice configuration:
...
The Ice harness snapshots native session JSONL before deleting its temporary workspace. An eval-only `afterEach` hook
