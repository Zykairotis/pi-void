...
const artifactDirectory = process.env.ICE_EVAL_ARTIFACT_DIR
	? resolve(packageRoot, process.env.ICE_EVAL_ARTIFACT_DIR)
	: resolve(
			packageRoot,
			".eval",
...
if (hasCliModelSelection) {
	...
	provider = process.env.ICE_PROVIDER?.trim() || undefined;
	model = process.env.ICE_MODEL?.trim() || undefined;
	if (Boolean(provider) !== Boolean(model)) {
		console.error("Default model selection requires both ICE_PROVIDER and ICE_MODEL.");
		process.exit(1);
	}
}
...
const childEnvironment = {
	...process.env,
	ICE_EVAL_ARTIFACT_DIR: artifactDirectory,
};
if (provider && model) {
	childEnvironment.ICE_PROVIDER = provider;
	childEnvironment.ICE_MODEL = model;
} else {
	delete childEnvironment.ICE_PROVIDER;
	delete childEnvironment.ICE_MODEL;
}
