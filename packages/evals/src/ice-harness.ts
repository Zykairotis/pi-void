...
import { contentText } from "@zykairotis/ice-ai";
import {
	type AgentSession,
...
import { ICE_SESSION_SNAPSHOT_ARTIFACT } from "./vitest-evals/artifacts.ts";

export type IceCodingAgentInput = string | Array<{ type: "prompt"; content: string } | { type: "reload" }>;

type IceCodingAgentModelSelection = {
	provider: string;
	id: string;
};

type IceCodingAgentHarnessOptions = {
	name?: string;
	model?: IceCodingAgentModelSelection;
	noTools?: CreateAgentSessionOptions["noTools"];
	transformSystemPrompt?: (defaultPrompt: string) => string;
};

type IceCodingAgentHarnessWithOutput<TOutput extends JsonValue> = IceCodingAgentHarnessOptions & {
	output: (args: { response: string; session: AgentSession }) => TOutput | Promise<TOutput>;
};

export function resolveModelSelection(
	explicitModel: IceCodingAgentModelSelection | undefined,
	environment: { ICE_PROVIDER?: string; ICE_MODEL?: string } = process.env,
): IceCodingAgentModelSelection {
	const provider = (explicitModel?.provider ?? environment.ICE_PROVIDER)?.trim();
	const id = (explicitModel?.id ?? environment.ICE_MODEL)?.trim();
	if (!provider || !id) {
		throw new Error("Select a harness model explicitly or set both ICE_PROVIDER and ICE_MODEL as defaults.");
	}
	return { provider, id };
}
...
async function runIceCodingAgent<TOutput extends JsonValue>(
	input: IceCodingAgentInput,
	signal: AbortSignal | undefined,
	setArtifact: HarnessContext["setArtifact"],
	options: IceCodingAgentHarnessOptions | IceCodingAgentHarnessWithOutput<TOutput>,
	...
	const root = await mkdtemp(join(tmpdir(), "ice-eval-"));
	...
	try {
		...
		try {
			...
			if (response === undefined) throw new Error("Ice eval input must include at least one prompt step.");
	...
	if (sessionManager) {
		try {
			...
			if (sessionPath && existsSync(sessionPath)) {
				setArtifact(ICE_SESSION_SNAPSHOT_ARTIFACT, await readFile(sessionPath, "utf8"));
			}
		} catch (error) {
...
export function createIceCodingAgentHarness<TOutput extends JsonValue>(
	options: IceCodingAgentHarnessWithOutput<TOutput>,
): Harness<IceCodingAgentInput, TOutput>;
export function createIceCodingAgentHarness(
	options?: IceCodingAgentHarnessOptions,
): Harness<IceCodingAgentInput, string>;
export function createIceCodingAgentHarness<TOutput extends JsonValue>(
	options: IceCodingAgentHarnessOptions | IceCodingAgentHarnessWithOutput<TOutput> = {},
) {
	return createHarness<IceCodingAgentInput, string | TOutput>({
		name: options.name ?? "ice-coding-agent",
		run: ({ input, signal, setArtifact }) => runIceCodingAgent(input, signal, setArtifact, options),
	});
}
