...
export const ICE_SESSION_SNAPSHOT_ARTIFACT = "iceSessionJsonl";

const evalSessionArtifactKey = Symbol("ice-evals-session-artifact");
const evalSourceArtifactKey = Symbol("ice-evals-source-artifact");

interface IceSessionAttachment extends TestAttachment {
	name: "session.jsonl";
...
interface IceSessionArtifact extends TestArtifactBase {
	type: "@zykairotis/ice-evals:session";
	runId: string;
	attachments: [IceSessionAttachment] | [];
}
...
declare module "vitest" {
	interface TestArtifactRegistry {
		[evalSessionArtifactKey]: IceSessionArtifact;
...
export async function recordEvalSessionArtifact(
	...
	const session = run.artifacts?.[ICE_SESSION_SNAPSHOT_ARTIFACT];
	if (session === undefined) return;
	if (typeof runId !== "string" || typeof session !== "string") {
		throw new TypeError("Ice eval session artifact metadata is invalid.");
	}
...
export async function persistEvalArtifactReferences(
	...
	for (const artifact of artifacts) {
		if (
			(artifact.type !== "@zykairotis/ice-evals:session" && artifact.type !== "@zykairotis/ice-evals:source") ||
			artifact.runId !== runId
		) {
			continue;
		}
		const category = artifact.type === "@zykairotis/ice-evals:session" ? "sessions" : "sources";
