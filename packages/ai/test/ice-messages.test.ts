...
function createModel(baseUrl: string): Model<"ice-messages"> {
	return {
		id: "auto",
		name: "Radius Auto",
		api: "ice-messages",
...
describe("ice-messages", () => {
	...
	it("appends debug=1 and reports response headers via onResponse", async () => {
		...
		const options: IceMessagesOptions = {
			apiKey: "test-key",
			debug: true,
			onResponse: (response) => {
		...
		expect(observedHeaders?.["x-ice-gateway-upstream-provider"]).toBe("anthropic");
	});
	...
	it("surfaces backend error responses with diagnostics", async () => {
		...
		expect(message.diagnostics?.[0]?.type).toBe("ice_messages_response_failure");
...
describe("ice-messages api registration", () => {
	it("is registered as a builtin api provider", async () => {
		const { getApiProvider } = await import("../src/compat.ts");
		expect(getApiProvider("ice-messages")).toBeDefined();
	});

	it("is a known api usable on models", () => {
		const api: Api = "ice-messages";
		expect(api).toBe("ice-messages");
	});
});
