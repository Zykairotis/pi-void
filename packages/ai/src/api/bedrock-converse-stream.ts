...
export const stream: StreamFunction<"bedrock-converse-stream", BedrockOptions> = (
	...
	(async () => {
		...
		// A profile explicitly configured through ice's auth flow (the `profile`
...
function addCustomHeadersMiddleware(client: BedrockRuntimeClient, headers: Record<string, string>): void {
	...
	client.middlewareStack.add(middleware, { step: "build", name: "ice-ai-custom-headers", priority: "low" });
}
...
function mapThinkingLevelToEffort(
	...
	if (typeof mapped === "string") return mapped as "low" | "medium" | "high" | "xhigh" | "max" | "ultra";
