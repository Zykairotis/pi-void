/**
 * ice-messages API implementation.
 *
 * Streams ice's own message protocol directly to a backend: the request is a
 ...
 * models.json custom provider with `"api": "ice-messages"`.
 */
...
type IceMessagesUsage = AssistantMessage["usage"];
type IceMessagesStopReason = AssistantMessage["stopReason"];

/** Impact summary of a server-side message rewrite (e.g. a gateway policy). */
export type IceMessagesRewriteImpact = {
	policyId: string;
...
/** Serialized assistant-message event as sent by a ice-messages backend. */
export type IceMessagesEvent =
	| { type: "start" }
	...
	| {
			type: "done";
			reason: Extract<IceMessagesStopReason, "stop" | "length" | "toolUse">;
			usage: IceMessagesUsage;
			responseId?: string;
			rewrite?: IceMessagesRewriteImpact;
	  }
	| {
			type: "error";
			reason: Extract<IceMessagesStopReason, "aborted" | "error">;
			usage: IceMessagesUsage;
			errorMessage?: string;
			responseId?: string;
			rewrite?: IceMessagesRewriteImpact;
...
export class IceMessagesResponseError extends Error {
	code?: string;
	...
	constructor(message: string, code: string | undefined, diagnosticDetails: Record<string, unknown>) {
		super(message);
		this.name = "IceMessagesResponseError";
		this.code = code;
		this.diagnosticDetails = diagnosticDetails;
	}
}

function parseIceMessagesErrorBody(body: string): IceMessagesErrorBody | undefined {
	try {
		const parsed = JSON.parse(body) as IceMessagesErrorBody | null;
...
function formatIceMessagesResponseError(
	response: Response,
	body: string,
	errorBody: IceMessagesErrorBody | undefined,
): string {
...
function createIceMessagesResponseError(
	model: Model<"ice-messages">,
	url: URL,
	response: Response,
	body: string,
): IceMessagesResponseError {
	const errorBody = parseIceMessagesErrorBody(body);
	const code = typeof errorBody?.error?.code === "string" ? errorBody.error.code : undefined;
	return new IceMessagesResponseError(formatIceMessagesResponseError(response, body, errorBody), code, {
		version: 1,
...
function createEmptyUsage(): IceMessagesUsage {
	return {
		input: 0,
		output: 0,
		cacheRead: 0,
...
function appendRewriteDiagnostic(message: AssistantMessage, rewrite: IceMessagesRewriteImpact | undefined): void {
	if (!rewrite) {
		return;
	}
	appendAssistantMessageDiagnostic(message, {
		type: "ice_messages_rewrite",
		timestamp: Date.now(),
		details: { ...rewrite },
	});
}

function createEventConverter(model: Model<"ice-messages">) {
	...
	return (event: IceMessagesEvent): AssistantMessageEvent => {
		switch (event.type) {
			case "done":
...
async function* readIceMessagesEvents(stream: ReadableStream<Uint8Array>): AsyncGenerator<IceMessagesEvent> {
	...
	try {
		while (true) {
			...
			while (split !== -1) {
				const event = parseIceMessagesEvent(buffer.slice(0, split));
				if (event) {
					yield event;
				}
		...
		if (buffer.trim()) {
			const event = parseIceMessagesEvent(buffer);
			if (event) {
				yield event;
			}
		}
...
function parseIceMessagesEvent(raw: string): IceMessagesEvent | undefined {
	const data = raw
		.split("\n")
	...
	return data && data !== "[DONE]" ? (JSON.parse(data) as IceMessagesEvent) : undefined;
}

function createErrorEvent(model: Model<"ice-messages">, error: unknown, aborted: boolean): AssistantMessageEvent {
	...
	if (!aborted && error instanceof IceMessagesResponseError) {
		appendAssistantMessageDiagnostic(
			assistantMessage,
			createAssistantMessageDiagnostic("ice_messages_response_failure", error, error.diagnosticDetails),
		);
	}
...
function resolveCacheRetention(cacheRetention?: CacheRetention, env?: ProviderEnv): CacheRetention | undefined {
	...
	return getProviderEnvValue("ICE_CACHE_RETENTION", env) === "long" ? "long" : undefined;
}

export const stream: StreamFunction<"ice-messages", IceMessagesOptions> = (
	model: Model<"ice-messages">,
	context: Context,
	options?: IceMessagesOptions,
	...
	void (async () => {
		try {
			...
			if (!response.ok) {
				...
				throw createIceMessagesResponseError(model, url, response, body);
			}
			if (!response.body) {
				throw new Error(`${model.provider} response has no body`);
			}

			for await (const iceEvent of readIceMessagesEvents(response.body)) {
				const event = convertEvent(iceEvent);
...
export const streamSimple: StreamFunction<"ice-messages", SimpleStreamOptions> = (
	model: Model<"ice-messages">,
	context: Context,
	options?: SimpleStreamOptions,
): AssistantMessageEventStream => {
	const extra = options as IceMessagesOptions | undefined;
