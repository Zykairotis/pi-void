...
Example extensions for ice-coding-agent.

## Usage

```bash
# Load an extension with --extension flag
ice --extension examples/extensions/permission-gate.ts

# Or copy to extensions directory for auto-discovery
cp permission-gate.ts ~/.ice/agent/extensions/
```
...
| `send-user-message.ts` | Demonstrates `ice.sendUserMessage()` for sending user messages from extensions |
...
| Extension | Description |
|-----------|-------------|
| `mac-system-theme.ts` | Syncs ice theme with macOS dark/light mode |
| `theming.ts` | Theme/color dropdowns with live preview in `/settings` (Theming) and `/theming`; custom colors save to `user.json` |
...
| `event-bus.ts` | Inter-extension communication via `ice.events` |
...
| `custom-provider-gitlab-duo/` | GitLab Duo provider using ice-ai's built-in Anthropic/OpenAI streaming via proxy |
