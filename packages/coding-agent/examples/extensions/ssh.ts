 ...
 *
 * Usage:
 *   ice -e ./ssh.ts --ssh user@host
 *   ice -e ./ssh.ts --ssh user@host:/remote/path
 *
 * Requirements:
...
export default function (ice: ExtensionAPI) {
	ice.registerFlag("ssh", { description: "SSH remote: user@host or user@host:/path", type: "string" });
	...
	ice.on("session_start", async (_event, ctx) => {
		...
		const arg = ice.getFlag("ssh") as string | undefined;
		if (arg) {
			if (arg.includes(":")) {
