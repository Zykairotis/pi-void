 ...
 * (logo + keybinding hints) with a custom component showing the ice mascot.
 */

import type { ExtensionAPI, Theme } from "@zykairotis/ice-coding-agent";
import { VERSION } from "@zykairotis/ice-coding-agent";

// --- ICE MASCOT ---
// Based on ice_mascot.ts - the ice agent character
function getIceMascot(theme: Theme): string[] {
	// --- COLORS ---
	// 3b1b Blue: R=80, G=180, B=230
	const iceBlue = (text: string) => theme.fg("accent", text);
...
export default function (ice: ExtensionAPI) {
			...
			ctx.ui.setHeader((_tui, theme) => {
				return {
					render(_width: number): string[] {
						const mascotLines = getIceMascot(theme);
	...
	ice.registerCommand("builtin-header", {
