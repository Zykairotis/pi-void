...
export default function (ice: ExtensionAPI) {
	...
	ice.on("session_start", async (event, ctx) => {
		ctx.ui.setTitle(event.reason === "new" ? "ice RPC Demo (new session)" : "ice RPC Demo");
	...
	ice.registerCommand("rpc-editor", {
	...
	ice.registerCommand("rpc-prefill", {
