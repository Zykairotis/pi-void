/**
 * Ice-vcc compile entry — orchestrates normalization → noise filtering → section building.
 *
 * Upstream: https://github.com/sting8k/ice-vcc (src/core/summarize.ts)
 * Unmodified.
 */
import type { Message } from "@zykairotis/ice-ai";
import type { FileOps } from "../types.ts";
import { buildSections } from "./build-sections.ts";
import { filterNoise } from "./filter-noise.ts";
import { capBrief, formatSummary, wrapLongLines } from "./format.ts";
...
const mergeHeaderSection = (header: string, prev: string, fresh: string): string => {
	...
	const capped =
		combined.length > CAP ? (header === "Session Goal" ? combined.slice(0, CAP) : combined.slice(-CAP)) : combined;
...
const stripOMContent = (text: string): string => {
	...
	if (reflIdx >= 0 || obsIdx >= 0) {
		...
		const minSectionIdx = Math.min(reflIdx >= 0 ? reflIdx : Infinity, obsIdx >= 0 ? obsIdx : Infinity);
