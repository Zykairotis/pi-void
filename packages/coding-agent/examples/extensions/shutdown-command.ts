 ...
 * Demonstrates how extensions can use ctx.shutdown() to exit ice cleanly.
 */
...
export default function (ice: ExtensionAPI) {
	// Register a /quit command that cleanly exits ice
	ice.registerCommand("quit", {
		description: "Exit ice cleanly",
	...
	ice.registerTool({
		...
		description: "Complete a task and exit ice",
	...
	ice.registerTool({
		...
		description: "Deploy the application and exit ice",
		parameters: Type.Object({
		...
		async execute(_toolCallId, params, _signal, onUpdate, ctx) {
			...
			// const result = await ice.exec("npm", ["run", "deploy", params.environment], { signal });
