 ...
 *
 * Runs ice's built-in tools inside a local Gondolin micro-VM. The host working
 ...
 * Usage:
 *   cd /path/to/project
 *   ice -e /path/to/ice/packages/coding-agent/examples/extensions/gondolin
 *
 * Requirements:
 *   - Node.js >= 23.6.0 for @zykairotis/ice-gondolin
...
import { RealFSProvider, VM } from "@zykairotis/ice-gondolin";
...
export default function (ice: ExtensionAPI) {
	...
	async function startVm(ctx?: ExtensionContext): Promise<VM> {
		...
		const created = await VM.create({
			sessionLabel: `ice ${path.basename(localCwd)}`,
			vfs: {
				mounts: {
