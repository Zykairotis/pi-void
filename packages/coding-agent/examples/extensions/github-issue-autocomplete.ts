...
async function resolveGitHubRepo(ice: ExtensionAPI, cwd: string): Promise<RepoResolution> {
	const result = await ice.exec("git", ["remote", "-v"], { cwd, timeout: 5_000 });
	if (result.code !== 0) {
...
export default function (ice: ExtensionAPI): void {
	ice.on("session_start", async (_event, ctx) => {
		const resolvedRepo = await resolveGitHubRepo(ice, ctx.cwd);
		if (!resolvedRepo.ok) {
