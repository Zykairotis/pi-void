...
afterEach(() => {
	...
	delete process.env.PI_BLACKHOLE_COMPACT_AFTER_PERCENT;
	delete process.env.PI_BLACKHOLE_COMPACT_AFTER_TOKENS;
});

function useConfig(raw: Record<string, unknown>): void {
	const directory = mkdtempSync(join(tmpdir(), "pi-blackhole-"));
	tempDirs.push(directory);
	mkdirSync(join(directory, "pi-blackhole"));
	writeFileSync(join(directory, "pi-blackhole", "pi-blackhole-config.json"), JSON.stringify(raw));
	process.env.PI_CODING_AGENT_DIR = directory;
}

describe("pi-blackhole unified config", () => {
	...
	it("writes only the selected threshold mode", () => {
		...
		const raw = JSON.parse(
			readFileSync(join(tempDirs[0], "pi-blackhole", "pi-blackhole-config.json"), "utf8"),
