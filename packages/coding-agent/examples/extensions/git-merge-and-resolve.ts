 ...
 *
 * Start ice with this extension:
 *   ice -e ./examples/extensions/git-merge-and-resolve.ts
 */
...
async function findConflicts(ice: ExtensionAPI, cwd: string): Promise<ConflictBlock[]> {
	const { stdout, code } = await ice.exec("git", ["diff", "--name-only", "--diff-filter=U"]);
...
export default function (ice: ExtensionAPI) {
	ice.on("agent_end", async (_event, ctx) => {
		const { code: revParseCode } = await ice.exec("git", ["rev-parse", "--git-dir"]);
		if (revParseCode !== 0) return;

		let ref = "MERGE_HEAD";

		// If not already in a merge, attempt one
		const { code: mergeHeadCode } = await ice.exec("git", ["rev-parse", "MERGE_HEAD"]);
		if (mergeHeadCode !== 0) {
			// Only attempt a new merge if the working tree is clean
			const { stdout: status } = await ice.exec("git", ["status", "--porcelain"]);
			if (status.trim()) return;

			const { stdout: upstream, code: upstreamCode } = await ice.exec("git", [
				"rev-parse",
				"--abbrev-ref",
				"--symbolic-full-name",
				"@{u}",
			...
			const { code: fetchCode, stderr: fetchErr } = await ice.exec("git", ["fetch", remote]);
			if (fetchCode !== 0) {
				ctx.ui.notify(`git-merge-and-resolve: fetch failed: ${fetchErr.trim()}`, "warning");
				return;
			}

			const { code: mergeCode } = await ice.exec("git", ["merge", "--no-ff", ref]);
			if (mergeCode === 0) return;
		}

		// Either we just merged with conflicts, or we were already in an unfinished merge
		const conflicts = await findConflicts(ice, ctx.cwd);
		if (conflicts.length === 0) return;

		ice.sendUserMessage(formatConflicts(ref, conflicts), { deliverAs: "followUp" });
	});
}
