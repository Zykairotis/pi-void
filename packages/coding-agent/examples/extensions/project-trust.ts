 ...
 *
 *   mkdir -p ~/.ice/agent/extensions
 *   cp packages/coding-agent/examples/extensions/project-trust.ts ~/.ice/agent/extensions/
 *
 * Or:
 *
 *   ice -e packages/coding-agent/examples/extensions/project-trust.ts
 *
 * Try it in a project containing .ice, AGENTS.md/CLAUDE.md, or .agents/skills.
 */

import type { ExtensionAPI, ProjectTrustEventResult } from "@zykairotis/ice-coding-agent";
...
export default function (ice: ExtensionAPI) {
	...
	ice.on("project_trust", async (event, ctx): Promise<ProjectTrustEventResult> => {
