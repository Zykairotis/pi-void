 ...
 *
 * Usage: ice --extension ./examples/extensions/overlay-qa-tests.ts
 *
 * Commands:
...
import type { ExtensionAPI, ExtensionCommandContext, Theme } from "@zykairotis/ice-coding-agent";
import type { Component, OverlayAnchor, OverlayHandle, OverlayOptions, TUI } from "@zykairotis/ice-tui";
import { Input, matchesKey, truncateToWidth, visibleWidth } from "@zykairotis/ice-tui";
...
export default function (ice: ExtensionAPI) {
	// Animation demo - proves overlays can handle real-time updates (like ice-doom would need)
	ice.registerCommand("overlay-animation", {
	...
	ice.registerCommand("overlay-anchors", {
	...
	ice.registerCommand("overlay-margins", {
	...
	ice.registerCommand("overlay-stack", {
	...
	ice.registerCommand("overlay-overflow", {
	...
	ice.registerCommand("overlay-edge", {
	...
	ice.registerCommand("overlay-percent", {
	...
	// Test maxHeight
	ice.registerCommand("overlay-maxheight", {
	...
	ice.registerCommand("overlay-sidepanel", {
	...
	ice.registerCommand("overlay-toggle", {
	...
	ice.registerCommand("overlay-passive", {
	...
	ice.registerCommand("overlay-focus", {
	...
	ice.registerCommand("overlay-streaming", {
	...
	private startProcess(): void {
		...
		this.proc = spawn("bash", [
			...
			for i in $(seq 1 100); do
				...
				DIR="/Users/nicobailon/Documents/development/ice-mono/packages/coding-agent/src/modes/interactive"
				...
				if [ $((i % 5)) -eq 0 ]; then
					echo -e "  \\033[32m✓ Successfully processed \${i} files in /Users/nicobailon/Documents/development/ice-mono\\033[0m"
				fi
				if [ $((i % 7)) -eq 0 ]; then
...
// Animation demo - proves overlays can handle real-time updates like ice-doom
class AnimationDemoComponent extends BaseOverlay {
	...
	render(width: number): string[] {
		...
		lines.push(border("│") + padLine(th.fg("dim", " (ice-doom uses same approach)")) + border("│"));
		lines.push(border("│") + padLine(``) + border("│"));
