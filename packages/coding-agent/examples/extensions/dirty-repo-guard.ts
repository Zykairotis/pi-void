...
async function checkDirtyRepo(
	...
	const { stdout, code } = await ice.exec("git", ["status", "--porcelain"]);
...
export default function (ice: ExtensionAPI) {
	ice.on("session_before_switch", async (event, ctx) => {
		...
		return checkDirtyRepo(ice, ctx, action);
	});

	ice.on("session_before_fork", async (_event, ctx) => {
		return checkDirtyRepo(ice, ctx, "fork");
	});
}
