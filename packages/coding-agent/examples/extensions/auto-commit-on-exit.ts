...
export default function (ice: ExtensionAPI) {
	ice.on("session_shutdown", async (_event, ctx) => {
		...
		const { stdout: status, code } = await ice.exec("git", ["status", "--porcelain"]);
		...
		const commitMessage = `[ice] ${firstLine.slice(0, 50)}${firstLine.length > 50 ? "..." : ""}`;

		// Stage and commit
		await ice.exec("git", ["add", "-A"]);
		const { code: commitCode } = await ice.exec("git", ["commit", "-m", commitMessage]);
