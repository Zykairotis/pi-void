 ...
 * - ~/.ice/agent/presets.json (global)
 * - <cwd>/.ice/presets.json (project-local)
 *
 ...
 *
 * Usage:
 * - `ice --preset plan` - start with plan preset
...
import { CONFIG_DIR_NAME, DynamicBorder, getAgentDir } from "@zykairotis/ice-coding-agent";
import { Container, Key, type SelectItem, SelectList, Text } from "@zykairotis/ice-tui";
...
export default function presetExtension(ice: ExtensionAPI) {
	...
	async function applyPreset(name: string, preset: Preset, ctx: ExtensionContext): Promise<boolean> {
		...
		if (activePresetName === undefined) {
			originalState = {
				model: ctx.model,
				thinkingLevel: ice.getThinkingLevel(),
				tools: ice.getActiveTools(),
			};
		}
		...
		if (preset.thinkingLevel) {
			ice.setThinkingLevel(preset.thinkingLevel);
		}

		// Apply tools if specified
		if (preset.tools && preset.tools.length > 0) {
			const allToolNames = ice.getAllTools().map((t) => t.name);
		...
		if (result === "(none)") {
			...
			if (originalState) {
				if (originalState.model) {
					await ice.setModel(originalState.model);
				}
				ice.setThinkingLevel(originalState.thinkingLevel);
				ice.setActiveTools(originalState.tools);
			} else {
				ice.setActiveTools(["read", "bash", "edit", "write"]);
			}
	...
	ice.registerShortcut(Key.ctrlShift("u"), {
	...
	ice.on("before_agent_start", async (event) => {
	...
	ice.on("session_start", async (_event, ctx) => {
		...
		const presetFlag = ice.getFlag("preset");
	...
	ice.on("turn_start", async () => {
		if (activePresetName) {
			ice.appendEntry("preset-state", { name: activePresetName });
		}
	});
}
