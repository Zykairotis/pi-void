...
import type { Component, EditorTheme, TUI } from "@zykairotis/ice-tui";
import { truncateToWidth, visibleWidth } from "@zykairotis/ice-tui";
...
export default function (ice: ExtensionAPI) {
	...
	ice.on("session_start", (_event, ctx) => {
		...
		const refreshBranch = async () => {
			const result = await ice.exec("git", ["branch", "--show-current"], { cwd: ctx.cwd }).catch(() => undefined);
		...
		class BorderStatusEditor extends CustomEditor {
			...
			render(width: number): string[] {
				...
				const thinking = ice.getThinkingLevel();
