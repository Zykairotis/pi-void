 ...
 * to the LLM. Custom entries are stored in the session via ice.appendEntry() and
 * rendered in interactive mode via ice.registerEntryRenderer().
 *
...
export default function (ice: ExtensionAPI) {
	ice.registerEntryRenderer<StatusCardData>("status-card", (entry, { expanded }, theme) => {
	...
	ice.registerCommand("status-card", {
		description: "Render a durable status card that is not sent to the LLM",
		handler: async (args) => {
			ice.appendEntry<StatusCardData>("status-card", {
