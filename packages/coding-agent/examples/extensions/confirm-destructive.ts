...
import type { ExtensionAPI, SessionBeforeSwitchEvent, SessionMessageEntry } from "@zykairotis/ice-coding-agent";

export default function (ice: ExtensionAPI) {
	ice.on("session_before_switch", async (event: SessionBeforeSwitchEvent, ctx) => {
		if (!ctx.hasUI) return;
