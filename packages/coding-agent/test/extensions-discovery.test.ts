...
describe("extensions discovery", () => {
	...
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "ice-ext-test-"));
	...
	const extensionCode = `
		export default function(ice) {
			ice.registerCommand("test", { handler: async () => {} });
		}
	`;
	...
	it("loads the coding-agent entrypoint without rewriting ice-ai provider subpaths", async () => {
		fs.writeFileSync(
	...
	it("keeps the type-only ice-ai OAuth compatibility barrel resolvable", async () => {
		fs.writeFileSync(
			path.join(extensionsDir, "oauth-import.ts"),
			`
				import * as oauth from "@zykairotis/ice-ai/oauth";
				void oauth;
				export default function(ice) {
					ice.registerCommand("test", { handler: async () => {} });
				}
	...
	it("discovers subdirectory with package.json ice field", async () => {
	...
	it("keeps package.json ice extension entries with leading tilde package-relative", async () => {
	...
	it("package.json with ice field takes precedence over index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
	...
	it("ignores package.json without ice field, falls back to index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
		fs.writeFileSync(path.join(subdir, "index.ts"), extensionCode);
		fs.writeFileSync(
	...
	it("handles mixed direct files and subdirectories", async () => {
		...
		fs.writeFileSync(path.join(subdir2, "package.json"), JSON.stringify({ ice: { extensions: ["./entry.ts"] } }));
	...
	it("registers message and entry renderers", async () => {
		const extCode = `
			export default function(ice) {
				ice.registerMarkdownTransformer((markdown) => {
					return markdown;
				});
				ice.registerMessageRenderer("my-custom-type", (message, options, theme) => {
					return null; // Use default rendering
				});
				ice.registerEntryRenderer("my-entry-type", (entry, options, theme) => {
					return null;
				});
			}
		`;
	...
	it("reports error when extension has no default export", async () => {
		const extCode = `
			export function notDefault(ice) {
				ice.registerCommand("test", { handler: async () => {} });
			}
		`;
	...
	it("loads extension with event handlers", async () => {
		const extCode = `
			export default function(ice) {
				...
				ice.on("tool_call", async (event) => undefined);
				ice.on("agent_end", async () => {});
			}
		`;
