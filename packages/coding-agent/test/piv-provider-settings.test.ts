...
import pivProviderSettings, {
	createProvidersSubmenu,
	formatProviderDescription,
} from "../src/piv-provider-settings.ts";
...
describe("piv provider settings", () => {
	beforeAll(() => {
		initTheme("dark");
	...
	it("registers a Providers submenu on the hidden piv extension", async () => {
		const loaded = await createTestExtensionsResult([
			{ name: "piv-provider-settings", factory: pivProviderSettings },
		]);
