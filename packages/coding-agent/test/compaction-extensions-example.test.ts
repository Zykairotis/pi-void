...
vi.mock("@zykairotis/ice-coding-agent", () => ({
...
describe("Documentation example", () => {
	it("custom compaction example should type-check correctly", () => {
		...
		const exampleExtension = (ice: ExtensionAPI) => {
			ice.on("session_before_compact", async (event: SessionBeforeCompactEvent, ctx) => {
	...
	it("compact event should have correct fields", () => {
		const checkCompactEvent = (ice: ExtensionAPI) => {
			ice.on("session_compact", async (event: SessionCompactEvent) => {
