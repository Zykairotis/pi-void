...
async function createWorkspace(): Promise<string> {
	const cwd = await mkdtemp(join(tmpdir(), "piv-subagents-adversarial-"));
	tempDirs.push(cwd);
	await mkdir(join(cwd, "src"));
	return cwd;
}

function request(cwd: string, role = "explore"): SubagentRequest {
	return {
...
function resolvedTask(cwd: string, id: string, role = "explore"): ResolvedSubagentBatchTask {
...
describe("Pi Void B8.1 deterministic adversarial invariants", () => {
	...
	it("scope.symlink-replacement rejects a selected resource replaced by an escaping symlink", async () => {
		...
		const agentDir = await mkdtemp(join(tmpdir(), "piv-adversarial-agent-"));
		const outside = await mkdtemp(join(tmpdir(), "piv-adversarial-outside-"));
		...
		const result = await runResolvedSubagentBatch(
			...
			["delegate", "read"],
			{
				runResolved: async (request) => {
					...
					return {
						...completedResult(task, { status: "failed", partial: false, observedOutputBytes: 15 * 1024 }),
						diagnostics: [{ code: "child_runtime_failure" as const, message: "temporary", retryable: true }],
					};
				},
			},
	...
	it("verification.oversized-evidence-array rejects more than the bounded finding count", async () => {
		const cwd = await createWorkspace();
		const task = resolvedTask(cwd, "oversized-findings", "review");
