...
import type { SubagentJobInspection, SubagentJobStatus } from "../src/piv-subagent-jobs.ts";
import {
...
function completionEntry(
	...
	return {
		type: "custom_message",
		customType: "piv-subagent-job-completion",
		timestamp,
...
describe("subagent observatory reducer", () => {
	it("tracks lifecycle, current child tool, usage, and normalized path", () => {
		...
		state = apply(
			...
			runtimeEvent("subagent_tool_start", "running", {
				nowMs: 1_500,
				currentPath: "/repo/src/piv-subagents.ts",
				event: { toolName: "edit" },
			}),
		);
		...
		expect(state.recent[0]).toMatchObject({
			...
			status: "completed",
			currentTool: "edit",
			currentPath: "src/piv-subagents.ts",
			elapsedMs: 1_000,
			usage,
		});
	});
	...
	it("renders bounded redacted progress without transcript content", () => {
		...
		expect(text).toContain("src/piv-subagents.ts");
