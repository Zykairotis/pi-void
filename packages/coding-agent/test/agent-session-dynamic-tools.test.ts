...
describe("AgentSession dynamic tool registration", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `ice-dynamic-tool-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("exposes session state before custom bash spawn hooks and supports opting out", async () => {
		...
		expect(sessionEnv).toMatchObject({
			ICE_SESSION_ID: session.sessionId,
			ICE_SESSION_FILE: session.sessionFile,
			ICE_PROVIDER: model.provider,
			ICE_MODEL: model.id,
			ICE_REASONING_LEVEL: session.thinkingLevel,
		});
		...
		expect(optedOutEnv).not.toHaveProperty("ICE_SESSION_ID");
		expect(optedOutEnv).not.toHaveProperty("ICE_SESSION_FILE");
		expect(optedOutEnv).not.toHaveProperty("ICE_PROVIDER");
		expect(optedOutEnv).not.toHaveProperty("ICE_MODEL");
		expect(optedOutEnv).not.toHaveProperty("ICE_REASONING_LEVEL");
