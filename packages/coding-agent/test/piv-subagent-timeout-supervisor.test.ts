...
} from "../src/piv-agent-view-bridge.ts";
import { SubagentJobRegistry, type SubagentJobRunResult } from "../src/piv-subagent-jobs.ts";
import {
	formatSubagentToolActivity,
	SubagentRunSupervisor,
	SubagentRunSupervisorRegistry,
} from "../src/piv-subagent-timeout-supervisor.ts";
import {
...
async function workspace(): Promise<{ cwd: string; agentDir: string }> {
	const cwd = await mkdtemp(join(tmpdir(), "piv-timeout-supervisor-"));
	const agentDir = await mkdtemp(join(tmpdir(), "piv-timeout-agent-"));
...
describe("Pi Void resumable timeout supervision", () => {
	...
	it("returns needs_time with the last three runtime-observed activities and resumes the same child", async () => {
		...
		expect(child.promptCalls[1]?.text).toContain("[PI VOID SUBAGENT CONTINUE]");
	...
	it("lets the expanded agent shelf extend or stop a selected needs-time child without switching views", async () => {
		...
		const control: PivAgentViewLiveSessionControl = {
			...baseControl,
			getRuntimeAttention: () => ({
...
describe("Pi Void timeout attention enrichment", () => {
	...
	it("normalizes legacy completed outcomes to ok at the bridge boundary", () => {
		const presentation = normalizePivAgentViewPresentation({
			runtimeAttention: {
				phase: "working",
				state: "awaiting_extension",
				initialTimeoutMs: 60_000,
