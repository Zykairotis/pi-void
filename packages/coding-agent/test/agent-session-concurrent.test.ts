...
describe("AgentSession concurrent prompt guard", () => {
	...
	beforeEach(async () => {
		tempDir = join(tmpdir(), `ice-concurrent-test-${Date.now()}`);
	...
	it("should queue extension-origin steering messages while streaming", async () => {
		...
		const extensionsResult = await createTestExtensionsResult([
			(ice) => {
				(globalThis as typeof globalThis & { testExtensionApi?: unknown }).testExtensionApi = ice;
			},
			(ice) => {
		...
		ice!.sendUserMessage("Steer from extension", { deliverAs: "steer" });
