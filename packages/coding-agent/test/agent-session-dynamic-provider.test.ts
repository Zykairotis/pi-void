...
describe("AgentSession dynamic provider registration", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `ice-dynamic-provider-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("applies top-level registerProvider overrides to the active model", async () => {
		const session = await createSession([
			(ice) => {
				ice.registerProvider("anthropic", { baseUrl: "http://localhost:8080/top-level" });
			},
		]);
		...
		const session = await createSession([
			(ice) => {
				ice.on("session_start", () => {
					ice.registerProvider("anthropic", { baseUrl: "http://localhost:8080/session-start" });
				});
			},
		]);
	...
	it("registers native ice-ai providers during extension loading", async () => {
		const session = await createSession([
			(ice) => {
				ice.registerProvider(nativeAnthropicProvider("http://localhost:8080/native-top-level"));
			},
		]);
	...
	it("applies command-time registerProvider overrides without reload", async () => {
		const session = await createSession([
			(ice) => {
				ice.registerCommand("use-proxy", {
					description: "Use proxy",
					handler: async () => {
						ice.registerProvider("anthropic", { baseUrl: "http://localhost:8080/command" });
					},
				});
			},
		]);
	...
	it("registers native ice-ai providers at command time", async () => {
		const session = await createSession([
			(ice) => {
				ice.registerCommand("use-native", {
					description: "Use native provider",
					handler: async () => {
						ice.registerProvider(nativeAnthropicProvider("http://localhost:8080/native-command"));
					},
				});
			},
		]);
