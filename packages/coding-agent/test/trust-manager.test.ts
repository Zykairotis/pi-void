...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".ice", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".ice", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".ice", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".ice"), { recursive: true });
			writeFileSync(join(cwd, ".ice", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".ice"), { recursive: true, force: true });
