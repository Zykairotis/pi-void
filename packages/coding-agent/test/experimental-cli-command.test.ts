...
describe("experimental CLI commands", () => {
	test("selects ice mode and parses existing CLI arguments", () => {
		expect(
	...
	test("parses a server listener", () => {
		expect(experimentalCli.parse(["server", "--listen", "unix:///tmp/ice.sock"])).toEqual({
			ok: true,
			command: {
				command: "server",
	...
	test("leaves experimental-looking existing option values with the existing parser", () => {
		expect(experimentalCli.parse(["--system-prompt", "--listen", "unix:///tmp/ice.sock"])).toMatchObject({
			ok: true,
			command: {
				command: "ice",
				options: { systemPrompt: "--listen", messages: ["unix:///tmp/ice.sock"] },
			},
		});
	});
	...
	test("stops parsing command options when existing CLI arguments begin", () => {
		...
		expect(result).toMatchObject({
			ok: true,
			command: { command: "ice", options: { model: "claude-sonnet" } },
		});
		if (!result.ok || result.command.command !== "ice") return;
	...
	test("parses a client transport address", () => {
		expect(experimentalCli.parse(["client", "--connect", "unix:///tmp/ice.sock"])).toEqual({
			ok: true,
			command: {
				command: "client",
				connect: { transport: "unix", path: "/tmp/ice.sock" },
			},
		});
	});
	...
	test.each([[[]], [["server"]], [["client"]]] as const)(
		...
		(argv) => {
			...
			expect(result).toMatchObject({ ok: true, command: { command: argv[0] ?? "ice" } });
	...
	test("passes unknown options, file arguments, and the positional separator to the existing parser", () => {
		const result = experimentalCli.parse(["--unknown", "@prompt.md", "--", "--listen", "unix:///tmp/ice.sock"]);
		expect(result).toMatchObject({
			ok: true,
			command: { command: "ice", options: { fileArgs: ["prompt.md"] } },
		});
		if (!result.ok || result.command.command !== "ice") return;
		expect(result.command.options.unknownFlags.get("unknown")).toBe(true);
		expect(result.command.options.unknownFlags.get("listen")).toBe("unix:///tmp/ice.sock");
	});

	test.each([
		[
			["--listen", "unix:///tmp/ice.sock", "--listen", "unix:///tmp/ice-admin.sock"],
		...
		[["--listen", "/tmp/ice.sock"], 'Invalid --listen address "/tmp/ice.sock"'],
		...
		[["--listen", "unix:///tmp/ice.sock?wrong=value"], 'Invalid --listen address "unix:///tmp/ice.sock?wrong=value"'],
		[["--listen", "unix:///tmp/ice.sock#fragment"], 'Invalid --listen address "unix:///tmp/ice.sock#fragment"'],
		[["--listen", "unix:/tmp/ice.sock"], 'Invalid --listen address "unix:/tmp/ice.sock"'],
		[["--listen", "unix:///tmp/%00ice.sock"], 'Invalid --listen address "unix:///tmp/%00ice.sock"'],
		[
			["client", "--listen", "unix:///tmp/ice.sock"],
			"The experimental client command does not support existing CLI options yet",
		],
		[
			["server", "--connect", "unix:///tmp/ice.sock"],
	...
	test("treats command names after the first argument as existing CLI arguments", () => {
		expect(experimentalCli.parse(["--cwd", "/workspace", "server"])).toMatchObject({
			ok: true,
			command: { command: "ice", options: { messages: ["server"] } },
		});
	});
});
