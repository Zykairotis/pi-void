...
import { Agent, type AgentEvent, type AgentTool } from "@zykairotis/ice-agent-core";
import { type AssistantMessage, type AssistantMessageEvent, EventStream, getModel } from "@zykairotis/ice-ai/compat";
...
describe("AgentSession retry", () => {
	...
	beforeEach(async () => {
		tempDir = join(tmpdir(), `ice-retry-test-${Date.now()}`);
