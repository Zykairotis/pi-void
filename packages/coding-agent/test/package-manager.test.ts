...
describe("DefaultPackageManager", () => {
	...
	beforeEach(() => {
		previousOfflineEnv = process.env.ICE_OFFLINE;
		delete process.env.ICE_OFFLINE;
	...
	afterEach(() => {
		if (previousOfflineEnv === undefined) {
			...
			process.env.ICE_OFFLINE = previousOfflineEnv;
		}
		vi.restoreAllMocks();
		vi.unstubAllGlobals();
...
Content`,
		...
		it("should auto-discover root markdown skills from .ice skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .ice", async () => {
			const extDir = join(tempDir, ".ice", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".ice"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".ice", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".ice", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".ice", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".ice", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".ice", "prompts");
			mkdirSync(promptsDir, { recursive: true });
		...
		it("should resolve directory with package.json ice.extensions in extensions setting", async () => {
			// Create a package with ice.extensions in package.json
			...
			// Should find the extensions declared in package.json ice.extensions
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user .ice/agent skills", async () => {
			const skillPath = join(agentDir, "skills", "user-ice", "SKILL.md");
			mkdirSync(join(agentDir, "skills", "user-ice"), { recursive: true });
			writeFileSync(skillPath, "---\nname: user-ice\ndescription: user ice\n---\n");
		...
		it("should use the project .ice dir as baseDir for project .ice skills", async () => {
			const projectBaseDir = join(tempDir, ".ice");
			const skillPath = join(projectBaseDir, "skills", "project-ice", "SKILL.md");
			mkdirSync(join(projectBaseDir, "skills", "project-ice"), { recursive: true });
			writeFileSync(skillPath, "---\nname: project-ice\ndescription: project ice\n---\n");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".ice", "agent");
		...
		it("should dedupe user skill entries when ~/.ice/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .ice auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".ice\n");

			const skillDir = join(tempDir, ".ice", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("resolveExtensionSources", () => {
		...
		it("should handle directories with ice manifest", async () => {
		...
		it("should keep ice manifest entries with leading tilde package-relative", async () => {
	...
	describe("command spawning", () => {
		it("should preserve argv entries containing spaces", () => {
			...
			const valueWithSpace = "C:\\Users\\A B\\.ice\\npm";
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".ice", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
		...
		it("should install user npm packages into the ice-managed npm root", async () => {
			settingsManager = SettingsManager.inMemory({
				npmCommand: ["pnpm"],
	...
	describe("temporary install paths", () => {
		it("should place temporary npm packages under the agent temp extension folder", () => {
			...
			expect(installPath.startsWith(join(tmpdir(), "ice-extensions"))).toBe(false);
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .ice settings base", () => {
			...
			const rel = relative(join(tempDir, ".ice"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
	...
	describe("pattern filtering in ice manifest", () => {
	...
	describe("pattern filtering in package filters", () => {
		...
		it("should resolve autoload-disabled project package entries as deltas over global packages", async () => {
			const pkgDir = join(agentDir, "npm", "node_modules", "ice-tools");
			mkdirSync(join(pkgDir, "extensions"), { recursive: true });
			writeFileSync(join(pkgDir, "package.json"), JSON.stringify({ name: "ice-tools", version: "1.0.0" }));
			writeFileSync(join(pkgDir, "extensions", "foo.ts"), "export default function() {}");
			writeFileSync(join(pkgDir, "extensions", "bar.ts"), "export default function() {}");
			settingsManager.setPackages(["npm:ice-tools"]);
			settingsManager.setProjectPackages([
				{ source: "npm:ice-tools", autoload: false, extensions: ["-extensions/foo.ts"] },
			]);
			const runCommandSpy = vi
				.spyOn(packageManager as unknown as PackageManagerInternals, "runCommand")
		...
		it("should resolve autoload-disabled package entries as positive-only without a global package", async () => {
			...
			settingsManager.setProjectPackages([
				{ source: relative(join(tempDir, ".ice"), pkgDir), autoload: false, extensions: ["+extensions/foo.ts"] },
			]);
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
		...
		it("should respect package.json ice.extensions manifest in subdirectories", async () => {
	...
	describe("offline mode and network timeouts", () => {
		it("should update npm range packages using the configured spec", async () => {
			const installedPath = join(tempDir, ".ice", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@^1.0.0", "--prefix", join(tempDir, ".ice", "npm"), "--legacy-peer-deps"],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".ice", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".ice", "npm", "node_modules", "project-current");
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".ice", "npm", "node_modules", "example");
