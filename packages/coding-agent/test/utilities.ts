...
import type { OAuthCredentials } from "@zykairotis/ice-ai";
import { getModel, streamSimple } from "@zykairotis/ice-ai/compat";
import { builtinProviders } from "@zykairotis/ice-ai/providers/all";
...
// ============================================================================
// OAuth API key resolution from ~/.ice/agent/auth.json
// ============================================================================

const AUTH_PATH = join(homedir(), ".ice", "agent", "auth.json");
...
/**
 * Resolve API key for a provider from ~/.ice/agent/auth.json
 *
...
/**
 * Check if a provider has credentials in ~/.ice/agent/auth.json
 */
...
/** Path to the real ice agent config directory */
export const ICE_AGENT_DIR = join(homedir(), ".ice", "agent");

/**
 * Get an AuthStorage instance backed by ~/.ice/agent/auth.json
...
export async function createTestSession(options: TestSessionOptions = {}): Promise<TestSessionContext> {
	const tempDir = join(tmpdir(), `ice-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
