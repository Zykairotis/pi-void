...
import { InMemoryModelsStore } from "@zykairotis/ice-ai";
...
beforeEach(() => {
	allowNetwork();
	tempDir = join(tmpdir(), `ice-test-radius-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
describe("Radius provider", () => {
	it("restores the legacy credential catalog without network access", async () => {
		...
		expect(model).toMatchObject({ api: "ice-messages", baseUrl: "https://radius.example.com/v1" });
