...
import pivSubagents from "../src/piv-subagents.ts";
...
async function createHarness(options: HarnessOptions = {}) {
	const cwd = await mkdtemp(join(tmpdir(), "piv-delegate-mvp-"));
	roots.push(cwd);
...
describe("Pi Void M10/M11 delegate integration", () => {
	it("keeps delegate and --sub-yolo PIV-only", () => {
		...
		const piv = spawnSync(process.execPath, ["src/piv.ts", "--no-extensions", "--help"], {
			cwd: process.cwd(),
			env,
			encoding: "utf8",
		});
		...
		expect(piv.status).toBe(0);
		expect(`${piv.stdout}${piv.stderr}`).toContain("--sub-yolo");
	});
	...
	it("completes a real delegate with fresh context, provenance, and verified evidence", async () => {
		...
		expect(launch).toMatchObject({ profile: { name: "explore", source: "bundled" }, projectTrusted: false });
	...
	it("preserves review claims as unresolved evidence after the integrated call", async () => {
		...
		const result = resultOf(
			await harness.tools
				.get("delegate")!
				.execute("delegate-review", delegateParams({ role: "review" }), undefined, undefined, harness.context),
		);
		const details = result.details!;
		const verification = details.verification as Record<string, unknown>;
		expect(result.isError).toBe(false);
	...
	it("keeps --sub-yolo explicit and build-only", async () => {
		...
		expect(normal.content[0]!.text).toContain("explicit --piv-mode build");
		expect(asyncResult.isError).toBe(true);
		expect(asyncResult.content[0]!.text).toContain("explicit --piv-mode build");
	});
	...
	it("allows unsafe review and batch delegation without per-child confirmation", async () => {
		...
		const batch = resultOf(
			await harness.tools
				...
				.execute(
					"unsafe-batch",
					{ tasks: [{ id: "trace", role: "explore", task: "Trace source.", scope: { roots: ["src"] } }] },
					undefined,
					undefined,
					harness.context,
				),
