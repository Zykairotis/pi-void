...
describe("regression #2860: replaced session callbacks", () => {
	...
	async function createRuntimeForTest(extensionFactory: ExtensionFactory, responses: string[]) {
		const tempDir = join(tmpdir(), `ice-2860-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("rebinds before withSession, targets the replacement session, and invalidates stale ice/ctx", async () => {
		const events: string[] = [];
		let oldCtx: ExtensionCommandContext | undefined;
		let oldIce: ExtensionAPI | undefined;
		let oldSessionFile: string | undefined;
		let staleCtxThrows = false;
		let staleIceThrows = false;
					...
					handler: async (_args, ctx) => {
						...
						await ctx.newSession({
							...
							withSession: async (replacedCtx) => {
								...
								try {
									oldIce?.sendUserMessage("stale message");
								} catch {
									staleIceThrows = true;
								}
		...
		expect(staleIceThrows).toBe(true);
