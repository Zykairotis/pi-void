...
import type { FauxModelDefinition, FauxProviderRegistration, FauxResponseStep, Model } from "@zykairotis/ice-ai/compat";
import { registerFauxProvider, streamSimple } from "@zykairotis/ice-ai/compat";
...
function createTempDir(): string {
	const tempDir = join(tmpdir(), `ice-suite-${Date.now()}-${Math.random().toString(36).slice(2)}`);
