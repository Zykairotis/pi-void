...
import { type CredentialStore, createModels, type Provider } from "@zykairotis/ice-ai";
...
describe("AuthStorage", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `ice-test-auth-storage-${Date.now()}-${Math.random().toString(36).slice(2)}`);
