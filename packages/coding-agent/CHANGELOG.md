...
- Added W8.4 live child-session attachment in `/agents` and `/subagents`: owner-local lifecycle registration, configurable attach/expand keys, bounded redacted transcript rendering, and Esc detachment. Attachment is observational only; child steering, mutation, and result ingestion remain unavailable.
...
- Added a PIV-only hidden `delegate` tool for one foreground native read-only child session with bundled `explore`/`review` roles, stripped child resources, bounded typed results, parent verification, and deterministic cancellation/timeout.
- Extended explicit `--piv-mode build --piv-allow-bash --sub-yolo` host execution to confirmed foreground, durable async, batch, and review delegation paths for all resolved roles. Unsafe children now receive the full scoped built-in tool set available to the trusted parent (`read`, `grep`, `find`, `ls`, `bash`, `edit`, and `write`); child extensions, MCP, recursive delegation, and `delegate_write` remain disabled. It remains interactive-only, requires project trust and active parent Bash, disables retry for unsafe children, and is explicitly not a sandbox; host filesystem, process, network, credential, and descendant-cleanup isolation are not claimed. `delegate_write` remains worktree-isolated.
