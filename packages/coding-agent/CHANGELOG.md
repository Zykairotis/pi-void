...
- Removed subagent request, batch-task, review-task, profile, imported-agent-pack, and reviewer-policy model selection. Every Pi Void child now uses the exact current parent model; durable async jobs capture it at acceptance time.
...
- Added switchable `/agents` transcript views: `/agents` opens a full-screen live child transcript and `/agents split` shows the parent and selected child side by side; Space switches the active pane and both views reuse Pi's normal message and tool renderers.
...
- Added PIV-only hidden delegation tools with a bundled/configured profile catalog, native child sessions, stripped child resources, parent-policy capability derivation, bounded typed results, parent verification, and deterministic cancellation/timeout.
...
- Added B7.1 bounded transient recovery across `delegate`, `delegate_batch`, and `review_batch`: one same-model retry for explicitly typed transient provider/startup failures, immutable normalized request/context reuse, aggregate usage with per-attempt output provenance, logical-task fail-fast, and typed attempt metadata. No fallback model is selected.
...
- Added Cognee `$project` datasets (`piv-<repo>-<hash>` from the git root) so automatic memory is not shared across repositories.
...
- Added a `piv` `/settings` → Providers submenu that lists configured providers and refetches a single catalog on demand. The `local` provider rewrites `models.json` from the 9Router endpoint.

### Changed

- Renamed the bundled Pi Void `scout` subagent profile to `explore`; `scout` remains an alias for existing prompts and configurations.
- `piv` no longer fetches the local 9Router model catalog on every launch. Startup uses the last valid `models.json` and injects the local API key only; use `/settings` → Providers → Refetch catalog to update.
- Changed Cognee `compactionSummaryMode: "auto"` so it never replaces the Pi compact summary. Explicit `own` now summarizes `messagesToSummarize` locally and skips network on overflow recovery.
...
- Changed Blackhole `tailBehavior: "minimal"` from unused config into a real cut: the kept Pi tail is summarized and dropped so compact no longer leaves ~20k recent tokens in context. Minimal is now the Blackhole default.

### Fixed

- Fixed `--sub-yolo` children being told to `read` loaded skills, then failing on the advertised `SKILL.md` path. Child `read`/`grep`/`find`/`ls` now follow skill-directory symlink aliases when the canonical directory is an exact loaded-skill read root, including ambient YOLO skills, without widening repository or mutation scope. Missing in-scope skill files still fail as `ENOENT`.
- Fixed Blackhole `tailBehavior` being stored but never applied, which left Pi's full `keepRecentTokens` tail after compact.
- Fixed `piv list`, `piv update`, `piv install`, and `piv config` being treated as chat prompts because the launcher prepends `--piv-mode build`.
...
- Fixed W6 writer integration to read the verifier configuration across the scoped Pi extension APIs, preserving the required verify-and-rollback path in live sessions.
- Clarified the Pi Void V1 release policy: B8 harness correctness and automated writer validation gate the implementation freeze, while exhaustive benchmark matrices and W5B live provider runs remain optional external certification evidence.
- Fixed PIV child delegation to enforce approved scope at the `read`, `grep`, `find`, and `ls` tool boundary, including absolute paths, `~` expansion, symlinked parents, and cross-volume containment checks; completed reports now require bounded structured in-scope evidence before parent verification can pass.
...
- Fixed Phase A resource handoff to exclude the Pi agent directory from user context roots, permit selected skills to load through exact resource read roots, pass bounded selected prompt content into child execution, cap selected resources at 64 KiB per file and 256 KiB in aggregate before reading, and use one resolved launch contract for execution and provenance.
...
- Fixed every Pi Void child path to use the exact current parent model object. Foreground, batch, review, and writer launches capture it at tool launch; durable async jobs capture it at acceptance time, including while queued.
