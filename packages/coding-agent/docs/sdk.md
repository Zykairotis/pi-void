> ice can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to ice's agent capabilities. Use it to embed ice in other applications, build custom interfaces, or integrate with automated workflows.
...
```bash
npm install @zykairotis/ice-coding-agent
```
...
```typescript
import { createAgentSession, SessionManager } from "@zykairotis/ice-coding-agent";
...
**Behavior:**
- **Extension commands** (e.g., `/mycommand`): Execute immediately, even during streaming. They manage their own LLM interaction via `ice.sendMessage()`.
...
The `Agent` class (from `@zykairotis/ice-agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.ice/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.ice/extensions/`)
- Project skills:
  - `.ice/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.ice/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.ice/agent/skills/`)
  - `~/.agents/skills/`
...
```typescript
import { getModel } from "@zykairotis/ice-ai";
import { ModelRuntime } from "@zykairotis/ice-coding-agent";
...
```typescript
import { InMemoryCredentialStore } from "@zykairotis/ice-ai";
import { createAgentSession, ModelRuntime } from "@zykairotis/ice-coding-agent";

// Default: uses ~/.ice/agent/auth.json and ~/.ice/agent/models.json
const modelRuntime = await ModelRuntime.create();
...
// Or inject any ice-ai CredentialStore
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@zykairotis/ice-coding-agent";
...
The `edit` tool returns `details.diff` for Ice's TUI display and `details.patch` as a standard unified patch for SDK consumers.

```typescript
import { createAgentSession } from "@zykairotis/ice-coding-agent";
...
import { createAgentSession, defineTool } from "@zykairotis/ice-coding-agent";
...
Use `defineTool()` for standalone definitions and arrays like `customTools: [myTool]`. Inline `ice.registerTool({ ... })` already infers parameter types correctly.

Custom tools passed via `customTools` are combined with extension-registered tools. Extensions loaded by the ResourceLoader can also register tools via `ice.registerTool()`.
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.ice/agent/extensions/`, `.ice/extensions/`, and settings.json extension sources.
...
```typescript
import type { InlineExtension } from "@zykairotis/ice-coding-agent";
...
**Event Bus:** Extensions can communicate via `ice.events`. Pass a shared `eventBus` to `DefaultResourceLoader` if you need to emit or listen from outside:

```typescript
import { createEventBus, DefaultResourceLoader } from "@zykairotis/ice-coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@zykairotis/ice-coding-agent";
...
1. Global: `~/.ice/agent/settings.json`
2. Project: `<cwd>/.ice/settings.json`
...
// Auth and Models
ModelRuntime // implements ice-ai Models and owns credential storage
