...
import { type ImageContent, type Message, type TextContent, type Usage, uuidv7 } from "@zykairotis/ice-ai";
...
export interface CompactionEntry<T = unknown> extends SessionEntryBase {
	...
	usage?: Usage;
	/** True if generated by an extension, undefined/false if ice-generated (backward compatible) */
	fromHook?: boolean;
}
...
export interface BranchSummaryEntry<T = unknown> extends SessionEntryBase {
	...
	usage?: Usage;
	/** True if generated by an extension, false if ice-generated */
	fromHook?: boolean;
}
 ...
 * Encodes cwd into a safe directory name under ~/.ice/agent/sessions/.
 */
...
export class SessionManager {
	...
	private _setSessionFile(sessionFile: string, preloadedFileEntries?: FileEntry[]): void {
		...
		if (existsSync(this.sessionFile)) {
			...
			// non-empty but did not parse as a ice session, fail without modifying it.
			if (this.fileEntries.length === 0) {
				const explicitPath = this.sessionFile;
				if (statSync(explicitPath).size > 0) {
					throw new Error(`Session file is not a valid ice session: ${explicitPath}`);
				}
				this.newSession();
	...
	/** Explicit opt-in persistence barrier; ordinary session writes retain their lazy behavior. */
	flushDurably(): void {
		if (!this.persist || !this.sessionFile) {
			throw new Error("Durable session entries require a persistent session; --no-session is not supported.");
		}
		const fd = openSync(this.sessionFile, this.flushed ? "a" : "wx", 0o600);
		try {
			if (!this.flushed) {
				for (const entry of this.fileEntries) writeFileSync(fd, `${JSON.stringify(entry)}\n`);
				this.flushed = true;
			}
			fsyncSync(fd);
		} finally {
	 ...
	 * @param sessionDir Optional session directory. If omitted, uses default (~/.ice/agent/sessions/<encoded-cwd>/).
	 */
