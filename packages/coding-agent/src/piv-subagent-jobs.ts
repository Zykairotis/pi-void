...
} from "./piv-subagents.ts";
import { redactCredentialText } from "./utils/redact.ts";

export const JOB_ENTRY_TYPE = "piv-subagent-job-v1";
export const JOB_COMPLETION_MESSAGE_TYPE = "piv-subagent-job-completion";
...
export class SubagentJobRegistry {
	...
	restore(entries: readonly unknown[], deliveredJobIds: ReadonlySet<string> = new Set<string>()): string[] {
		...
		for (const entry of entries) {
			entryOrder++;
			if (!isRecord(entry) || entry.type !== "custom" || entry.customType !== JOB_ENTRY_TYPE) continue;
