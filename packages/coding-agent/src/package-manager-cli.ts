...
import { Markdown, type MarkdownTheme } from "@zykairotis/ice-tui";
...
import { getLatestIceRelease, isNewerPackageVersion } from "./utils/version-check.ts";
import {
...
function getPackageCommandUsage(command: PackageCommand): string {
	switch (command) {
		...
		case "update":
			return `${APP_NAME} update [source|self|ice] [--self|--extensions|--models|--all] [--extension <source>] [--approve|--no-approve] [--force]`;
		case "list":
...
Update ice, installed packages, or model catalogs.

Options:
  --self                  Update ice only (default when no target is given)
  --extensions            Update installed packages only
  --models                Refresh model catalogs only
  --all                   Update ice and installed packages
  --extension <source>    Update one package only
  -a, --approve           Trust project-local files for this command
  -na, --no-approve       Ignore project-local files for this command
  --force                 Reinstall ice even if the current version is latest

Short forms:
  ${APP_NAME} update                Update ice only
  ${APP_NAME} update --all          Update ice and all extensions
  ${APP_NAME} update --models       Refresh model catalogs only
  ${APP_NAME} update <source>       Update one package
  ${APP_NAME} update ice             Update ice only (self works as alias to ice)
`);
			return;
...
function stripLeadingIceModeArgs(args: string[]): string[] {
	const rest = [...args];
	while (rest.length > 0) {
		const arg = rest[0]!;
		if (arg === "--ice-mode" && rest[1] !== undefined && !rest[1].startsWith("-")) {
			rest.splice(0, 2);
			continue;
		}
		if (arg.startsWith("--ice-mode=")) {
			rest.shift();
			continue;
		}
		break;
...
function parsePackageCommand(args: string[]): PackageCommandOptions | undefined {
	const [rawCommand, ...rest] = stripLeadingIceModeArgs(args);
	...
	if (command === "update") {
		...
		if (modelsFlag) {
			...
			const sourceIsSelf = source === "self" || source === "ice";
			if (sourceIsSelf) {
...
function printSelfUpdateUnavailable(
	...
	if (entrypoint) {
		console.error("");
		console.error(`Location of ice executable: ${entrypoint}`);
	}
}
...
async function getSelfUpdatePlan(force: boolean): Promise<SelfUpdatePlan> {
	let latestRelease: Awaited<ReturnType<typeof getLatestIceRelease>>;
	try {
		latestRelease = await getLatestIceRelease(VERSION);
	} catch (error: unknown) {
...
export async function handleConfigCommand(
	...
	const [command, ...rest] = stripLeadingIceModeArgs(args);
	if (command !== "config") {
		return false;
	}
