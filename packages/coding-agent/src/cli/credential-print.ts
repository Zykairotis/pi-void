import type { Api, CredentialInfo, Model } from "@zykairotis/ice-ai";
...
export function printCredentialPrintHelp(): void {
	console.log(`Usage:
  ice auth print-api-key --model <model> [--provider <provider>]
  ice auth print-bearer-token --model <model> [--provider <provider>] [--min-expiry <duration>]
...
/** Parse the small, extensible `ice auth` command surface before normal startup. */
export function parseCredentialPrintCommand(args: string[]): CredentialPrintCommand | undefined {
	...
	if (!kind) {
		throw new CredentialPrintError(
			`Unknown auth command "${args[1] ?? ""}". Use "ice auth print-api-key" or "ice auth print-bearer-token".`,
		);
	}
