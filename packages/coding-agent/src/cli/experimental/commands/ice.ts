...
export interface IceCommand {
	readonly command: "ice";
	readonly auth?: AuthInput;
	readonly options: Args;
	readonly listen?: readonly TransportAddress[];
}

export interface IceCommandContext {
	runIce(command: IceCommand): void | Promise<void>;
}

const listenOption = transportOption("--listen");

export const iceCommand = new Command<IceCommand, IceCommandContext>("ice")
	.option(listenOption)
	...
	.action((command, context) => context.runIce(command));
