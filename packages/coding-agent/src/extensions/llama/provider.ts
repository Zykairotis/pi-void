...
import { stream, streamSimple } from "@zykairotis/ice-ai/compat";
...
function toIceModel(model: LlamaModelInfo, serverUrl: string): Model<"openai-completions"> {
...
export function createLlamaProvider(): LlamaProviderController {
	...
	const setCatalog = (catalog: readonly LlamaModelInfo[], serverUrl: string): void => {
		models = catalog.filter((model) => model.status.value === "loaded").map((model) => toIceModel(model, serverUrl));
	};
	...
	const provider: Provider<"openai-completions"> = {
		...
		refreshModels: async (context: RefreshModelsContext): Promise<void> => {
			...
			const refreshed = catalog
				...
				.map((model) => toIceModel(model, serverUrl));
			await context.publish({
