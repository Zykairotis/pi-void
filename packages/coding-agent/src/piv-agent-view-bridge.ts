...
import type { SubagentRuntimeAttention } from "./piv-subagent-timeout-supervisor.ts";
import { redactCredentialText } from "./utils/redact.ts";

export type PivAgentViewKind = "parent" | "subagent" | "historical-subagent";
export type PivAgentViewAuthority = "safe" | "yolo";

export interface PivAgentViewFinalResult {
...
/**
 * PIV-owned presentation metadata. It is a projection over the raw child
 * session and never replaces or mutates the messages used for verification.
 */
export interface PivAgentViewPresentation {
	readonly delegatedTask?: string;
	readonly scopeLabels?: readonly string[];
	readonly authority?: PivAgentViewAuthority;
	...
	readonly finalResult?: PivAgentViewFinalResult;
}

export type PivAgentViewPresentationPatch = PivAgentViewPresentation;

export type PivAgentViewInteractionMode = "mirror" | "controlled";
export type PivAgentViewControlState =
	| "working"
	| "awaiting-extension"
...
export interface PivAgentViewLiveSessionControl {
	readonly getState: () => PivAgentViewControlState;
...
export type PivAgentViewControlResult =
	| { readonly accepted: true; readonly mode: PivAgentViewInteractionMode }
	| { readonly accepted: false; readonly reason: string };

export interface PivAgentViewDescriptor {
	readonly kind: PivAgentViewKind;
	readonly id: string;
	...
	readonly interactionMode?: PivAgentViewInteractionMode;
	readonly controlState?: PivAgentViewControlState;
	readonly authority?: PivAgentViewAuthority;
...
export interface PivAgentViewLiveSession {
	readonly runId: string;
	readonly role: string;
	readonly taskId?: string;
	readonly model?: string;
...
export interface PivAgentViewLiveSessionSource {
	list(): readonly PivAgentViewLiveSession[];
...
export interface PivAgentViewSnapshotInput {
	readonly runId: string;
	readonly role: string;
	readonly model?: string;
	readonly status: string;
...
function normalizeRuntimeAttention(input: unknown): SubagentRuntimeAttention | undefined {
	...
	return deepFreezeSnapshot({
		...
		...(usage ? { usage } : {}),
	}) as PivAgentViewPresentation["runtimeAttention"];
}

/** Normalize presentation data at the bridge boundary before publishing it. */
export function normalizePivAgentViewPresentation(
	input: PivAgentViewPresentation | undefined,
	sourceIndices?: readonly number[],
): PivAgentViewPresentation | undefined {
	...
	const normalized: PivAgentViewPresentation = {
	...
	return Object.keys(normalized).length > 0 ? (deepFreezeSnapshot(normalized) as PivAgentViewPresentation) : undefined;
}

export class PivAgentViewBridge {
	private parentSession: AgentSession | undefined;
	private liveSource: PivAgentViewLiveSessionSource | undefined;
	private unsubscribeLiveSource: (() => void) | undefined;
	private readonly historical = new Map<string, PivAgentViewDescriptor>();
	private readonly listeners = new Set<() => void>();
	private readonly switcherRequestListeners = new Set<() => void>();
	private readonly uiState = new Map<string, AgentViewUiState>();
	private readonly interactionModes = new Map<string, PivAgentViewInteractionMode>();
	...
	connectLiveSessions(source: PivAgentViewLiveSessionSource): void {
	...
	registerHistoricalSnapshot(input: PivAgentViewSnapshotInput): void {
		const id = input.runId;
		const clonedMessages = cloneBoundedMessages(input.messages);
		const presentation = normalizePivAgentViewPresentation(input.presentation, clonedMessages.sourceIndices);
	...
	listViews(): readonly PivAgentViewDescriptor[] {
		const parent: PivAgentViewDescriptor[] = this.parentSession
			? [
					Object.freeze({
		...
		const live = liveEntries.map((entry) => {
			...
			return Object.freeze({
				...
				presentation: runtimeAttention
					? normalizePivAgentViewPresentation({ ...entry.presentation, runtimeAttention })
					: entry.presentation,
				model: entry.model,
				status:
	...
	getView(id: string): PivAgentViewDescriptor | undefined {
		const views = this.listViews();
		if (id === "parent") return views.find((view) => view.kind === "parent");
		return views.find((view) => view.id === id);
	}

	getDisplayedView(): PivAgentViewDescriptor | undefined {
	...
	getInteractionMode(id = this.displayedId): PivAgentViewInteractionMode | undefined {
		if (id === "parent") return undefined;
		return this.interactionModes.get(id) ?? (this.getView(id)?.kind === "subagent" ? "mirror" : undefined);
	}

	requestTakeControl(id = this.displayedId): PivAgentViewControlResult {
