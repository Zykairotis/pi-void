...
import { DynamicBorder, type ExtensionAPI, type ExtensionContext } from "@zykairotis/ice-coding-agent";
import { Container, hyperlink, Text } from "@zykairotis/ice-tui";
...
async function fetchAdvisoryMetadata(ice: ExtensionAPI, cwd: string, target: string): Promise<GhMetadata | undefined> {
	const advisoryRef = parseAdvisoryUrl(target) ?? (await readAdvisoryRefFromDraft(cwd, target));
	if (!advisoryRef) return undefined;

	try {
		const result = await ice.exec("gh", [
			"api",
...
async function fetchGhMetadata(
	...
	if (kind === "advisory") {
		return fetchAdvisoryMetadata(ice, cwd, target);
	}
	...
	try {
		const result = await ice.exec("gh", args);
...
export default function promptUrlWidgetExtension(ice: ExtensionAPI) {
	...
	const applySessionName = (ctx: ExtensionContext, match: PromptMatch, metadata?: GhMetadata) => {
		...
		const currentName = ice.getSessionName()?.trim();
		if (!currentName) {
			ice.setSessionName(desiredName);
			return;
		}
	...
	const updatePromptContext = (ctx: ExtensionContext, match: PromptMatch) => {
		...
		void fetchGhMetadata(ice, match.kind, match.target, ctx.cwd).then((meta) => {
	...
	ice.on("session_switch", async (_event, ctx) => {
