/**
 * Import a ice session shared as a gist by the issue-analysis CI workflow
 ...
 *   /ir https://ice.dev/session/#b4d100022aefb12f25dd2d8485e0a82a
 *   /ir https://github.com/earendil-works/ice/issues/123
 *
 *   ice "/ir <gist-id>"
 */
...
import type { ExtensionAPI, ExtensionCommandContext } from "@zykairotis/ice-coding-agent";

const GIST_ID_RE = /^[0-9a-fA-F]{20,}$/;
const GIST_URL_RE = /^https:\/\/gist\.github\.com\/(?:[^/]+\/)?([0-9a-fA-F]{20,})(?:[/#?].*)?$/;
const SHARE_URL_RE = /^https:\/\/ice\.dev\/session\/#([0-9a-fA-F]{20,})(?:[/#?].*)?$/;
...
function parseRef(
	...
	throw new Error(`expected a gist ID, gist URL, ice.dev share URL, issue URL, .html file, or .jsonl file: ${ref}`);
}
...
function decodeExportedHtml(html: string): { header: SessionHeader; jsonl: string } {
	...
	if (!match) throw new Error("HTML does not contain embedded ice session data");

	let data: unknown;
	try {
		data = JSON.parse(Buffer.from(match[1], "base64").toString("utf8"));
	} catch {
		throw new Error("embedded ice session data is not valid JSON");
	}
	...
	if (!header || header.type !== "session" || typeof header.id !== "string" || typeof header.cwd !== "string") {
		throw new Error("embedded ice session data has no valid session header");
	}
	if (!Array.isArray(sessionData.entries)) {
		throw new Error("embedded ice session data has no entries array");
	}
...
function getCiWorkdirName(sourceCwd: string): string | undefined {
	...
	return /^ice-ci-[0-9a-f]{32}$/i.test(name) ? name : undefined;
}
