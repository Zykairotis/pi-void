...
test("synchronizes private dependencies without touching registry aliases, generated manifests, or published lockstep", async () => {
	const root = await mkdtemp(join(tmpdir(), "ice-sync-versions-"));
	try {
		await writeManifest(root, "packages/ai", {
			name: "@zykairotis/ice-ai",
			version: "2.0.0",
		});
		await writeManifest(root, "packages/coding-agent", {
			name: "@zykairotis/ice-coding-agent",
			version: "2.0.0",
		});
		await writeManifest(root, "packages/evals", {
			name: "@zykairotis/ice-evals",
			version: "9.9.9",
			private: true,
			dependencies: {
				"@zykairotis/ice-coding-agent": "^1.0.0",
				"@example/vendored-ai": "npm:@zykairotis/ice-ai@1.0.0",
			},
		});
		...
		assert.equal(evalsManifest.dependencies["@zykairotis/ice-coding-agent"], "^2.0.0");
		assert.equal(evalsManifest.dependencies["@example/vendored-ai"], "npm:@zykairotis/ice-ai@1.0.0");
		const generatedManifest = await readManifest(root, "packages/coding-agent/install-lock");
		assert.equal(generatedManifest.dependencies["@zykairotis/ice-coding-agent"], "^1.0.0");
