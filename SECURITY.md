...
Ice and also where the boundaries are.

In general Ice is a coding agent that runs locally within the security boundary
...
Ice treats the local user account and files writable by that account as inside
the same trust boundary as the Ice process itself.  If an attacker can modify files
under the user's home directory, workspace, shell startup files, environment, or
Ice configuration, they can generally influence Ice or other local developer tools.
Reports that depend on such prior local write access are not security
vulnerabilities unless they demonstrate how Ice grants that write access or crosses
an operating-system privilege boundary.

Ice relies on users installing trustworthy extensions and loading trustworthy
skills and only to use ice within trusted repositories.  This is because files
...
If you believe you found a security vulnerability in ice or another package in
...
- Local code execution or sandboxing behavior (the Ice coding agent intentionally does not have a sandbox)
- Behavior of ice extensions or skills installed by the user
...
- Public internet exposure of a Ice installation
...
- Reports requiring the ability to create, modify, delete, or replace files,
  ...
  user-controlled local state on the target machine. This includes `~/.ice`,
  `~/.ice/agent/models.json`, workspace files, `AGENTS.md`, skills, extensions,
  extension configuration, dotfiles, and files synchronized through NFS, roaming
  profiles, or dotfile managers, unless the report shows how Ice itself grants
  that access.
- Issues caused by intentionally weakened user configuration.
- Resource/DOS claims that require trusted local input/config against the ice coding agent.
...
For example, a report showing that malicious contents written to a trusted Ice
configuration file cause Ice to execute commands, load attacker-controlled tools,
