# Pi-Void Verified Flaw Remediation Plan

**Date:** 2026-08-31  
**Repository:** `/home/mewtwo/ZSSD/pi-void`  
...
- `agent_docs/piv-codebase-audit-10-flaws.md`
...
The fixes must not weaken any Pi-Void safety property:
...
5. run cross-cutting Pi-Void tests affected by the change;
...
`packages/coding-agent/test/suite/regressions/piv-edit-exact-before-fuzzy.test.ts`
...
**File:** `packages/coding-agent/src/piv-cognee.ts`
...
- `savePivCogneeConfig()` around lines 510-534
...
Prefer one local helper in `piv-cognee.ts` rather than four copy-pasted `try/finally` blocks.
...
1. recognize only temp names generated by Pi-Void's own pending-record pattern, e.g. `*.json.<uuid>.tmp`;
...
Extend `packages/coding-agent/test/piv-cognee.test.ts`.
...
8. `savePivCogneeConfig()` failure cleans temp;
9. session-map best-effort failure cleans temp while preserving best-effort return behavior;
10. stale-temp cleanup removes only eligible old Pi-Void temp files and never durable JSON records, if stale cleanup is implemented.
...
**File:** `packages/coding-agent/src/piv-subagent-jobs.ts`
...
For Pi-Void, adopt this explicit rule:
...
Extend `packages/coding-agent/test/piv-subagent-jobs.test.ts`.
...
**File:** `packages/coding-agent/src/piv-cognee.ts`, `runImprove()` around lines 996-1023.
...
**File:** `packages/coding-agent/src/piv-subagents.ts`
...
`piv-subagents.test.ts` — `restores owner-scoped jobs and delivers only undelivered completion metadata`
...
- `pi.appendEntry(JOB_ENTRY_TYPE, snapshot)` -> durable `type: "custom"` authoritative job state;
- `pi.sendMessage(..., { triggerTurn: false })` -> runtime message that is persisted through `appendCustomMessageEntry()` as `type: "custom_message"`;
...
**File:** `packages/coding-agent/src/piv-safe-verify.ts`
...
5. run `piv-subagent-jobs.test.ts` plus subagent integration tests.
...
npm exec vitest -- run test/suite/regressions/piv-edit-exact-before-fuzzy.test.ts
npm exec vitest -- run test/piv-cognee.test.ts
npm exec vitest -- run test/piv-subagent-jobs.test.ts
```

Relevant protection tests for rejected claims:

```bash
npm exec vitest -- run test/piv-subagents.test.ts -t "restores owner-scoped jobs and delivers only undelivered completion metadata"
npm exec vitest -- run test/piv-subagents.test.ts -t "admits a retry only after reconciling the first attempt"
npm exec vitest -- run test/piv-safe-verify.test.ts -t "maps pass, failure, timeout, cancellation, spawn errors, and bounded output"
```
...
- `packages/coding-agent/test/suite/regressions/3302-find-path-glob.test.ts`
- new focused edit regression test, preferably `packages/coding-agent/test/suite/regressions/piv-edit-exact-before-fuzzy.test.ts`
- `packages/coding-agent/test/piv-cognee.test.ts`
- `packages/coding-agent/test/piv-subagent-jobs.test.ts`
