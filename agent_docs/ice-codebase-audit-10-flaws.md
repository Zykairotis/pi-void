# ICE Codebase Audit — 10 Flaws

Read-only audit of the ICE monorepo (`packages/coding-agent/src`). Each flaw was
...
**File:** `packages/coding-agent/src/ice-subagents.ts:6137-6165`

```ts
const deliverCompletion = (jobId: string): void => {
  if (shuttingDown || parentBusy || !jobs) return;
  ...
  ice.sendMessage({ customType: JOB_COMPLETION_MESSAGE_TYPE, ... }, { triggerTurn: false });
};
...
**Files:** `packages/coding-agent/src/ice-subagents.ts:7218-7229`, `ice-subagent-observatory.ts:307`

- Job snapshots are persisted via `ice.appendEntry(JOB_ENTRY_TYPE, snapshot)` (line 7229),
...
- `projectSubagentCompletionInbox` (ice-subagent-observatory.ts:307) also only matches
  `custom_message`.
...
**File:** `packages/coding-agent/src/ice-cognee.ts:389-404`
...
`.tmp` files accumulate unbounded in `~/.ice/agent/ice-cognee/pending/`.
...
**File:** `packages/coding-agent/src/ice-cognee.ts:892-911`
...
**File:** `packages/coding-agent/src/ice-cognee.ts:167-172`
...
export function cogneeSessionId(hostSessionId: string): string {
  ...
  return cleaned.startsWith("ice_") ? cleaned : `ice_${cleaned || "session"}`;
}
```
...
**File:** `packages/coding-agent/src/ice-subagents.ts:6827-6848`
...
**File:** `packages/coding-agent/src/ice-safe-verify.ts:466-546` (`execVerifier`), `528-605` (`runVerifier`)
...
misclassified as generic failures, corrupting the `ice-verify` status shown to the user.

---

## 8. `projectResult` discards a completed result when `requestedStop` is set — MEDIUM (correctness)

**File:** `packages/coding-agent/src/ice-subagent-jobs.ts:456-464`
...
**File:** `packages/coding-agent/src/ice-subagents.ts:4800-4807`
...
**File:** `packages/coding-agent/src/ice-cognee.ts:1791-1798`, `ice-cognee-client.ts:296-317`
...
`client.improve()` uses `options.timeoutMs ?? 120_000` (ice-cognee-client.ts:316) — a
...
to 120 s. After that, `waitForBackground` (ice-cognee.ts:664-667) loops while
...
The `drainPending` guard `if (runtime.shuttingDown) return` (ice-cognee.ts:1034) means
