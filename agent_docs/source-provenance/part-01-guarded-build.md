...
`agent_references/` is read-only. Ice source is implementation authority. External sources below informed behavior or tests; no exact external code was copied.

| Local component | Reference project | Source path | Snapshot identity | License | Reuse type |
|---|---|---|---|---|---|
| Hidden extension and lifecycle integration | Ice | `packages/coding-agent/src/core/extensions/types.ts`, `runner.ts`, `agent-session.ts`, `examples/extensions/` | `7f82eed08c19099c663eb8c1ccb5fbac72dc0b86` | MIT | direct native use |
...
- Bash: build mode plus current-process `--ice-allow-bash` plus project trust.
...
- Verifier argv authority: current-process `--ice-verify` only.
